...
function UpdateStudentModal({ onClose, openPopUp, updateId, getStudents }) {
  ...
  useEffect(() => {
    getStudentById(updateId);
  }, [updateId]);

  const getStudentById = async (id) => {
    const request = await studentServices.getStudentById(id);
    setName(request.result[0].NAME);
    setEmail(request.result[0].EMAIL);
    setBirthDate(request.result[0].BIRTHDATE);
  };

  const onChange = (e) => {
    if (e.target.id === "nome") setName(e.target.value);
    if (e.target.id === "email") setEmail(e.target.value);
    if (e.target.id === "dataNascimento") setBirthDate(new Date(e.target.value).toLocaleDateString());
  };

  const updateStudent = async () => {
    setIsLoading(true)
    const msg = await studentServices.updateStudent(
      name,
      email,
      birthdate,
      updateId
